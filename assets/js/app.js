(function () {
  ...
  var LOCALES = {
    fr: {
      ...
      code_label: "Code",
      use_model_label: "Modèles compatibles",
      use_model_unavailable: "Modèles compatibles indisponibles",
    ...
    en: {
      ...
      code_label: "Code",
      use_model_label: "Use Models",
      use_model_unavailable: "Use Models unavailable",
      latest_price_label: "Prices",
      price_unavailable: "Prices unavailable",
    ...
    ar: {
      ...
      code_label: "الرمز",
      use_model_label: "طرازات الاستخدام",
      use_model_unavailable: "طرازات الاستخدام غير متوفرة",
      latest_price_label: "Prices",
      price_unavailable: "Prices unavailable",
  ...
  function getPartUseModels(part) {
    return cleanText(part && (part.useModels || part.sourceModel));
  }

  function formatUseModels(part) {
    return getPartUseModels(part) || t("use_model_unavailable");
  }
    ...
    return '' +
      '<article class="part-row" data-part-code="' + escapeHtml(part.code) + '">' +
        ...
        '<div class="part-row__body">' +
          ...
          '<div class="part-row__use-model">' + t("use_model_label") + ': ' + escapeHtml(formatUseModels(part)) + '</div>' +
      ...
      return '' +
        '<article class="cart-item">' +
          ...
          '<div class="cart-item__body">' +
            ...
            '<div class="cart-item__use-model">' + t("use_model_label") + ': ' + escapeHtml(formatUseModels(item)) + '</div>' +
  ...
  function buildWhatsAppMessage(items, modelName) {
    var lines = items.map(function (item, index) {
      ...
      return (index + 1) + ". " + getPartMessageName(item) + " | " + item.code + " | Use Models: " + (getPartUseModels(item) || "N/A") + " | Qty: " + item.quantity + " | Prices: " + formatLatestPriceForMessage(item) + " | Subtotal: " + subtotal;
    });
  ...
  function submitCart() {
    ...
    var modelName = state.catalog.model.name || "ST";
      ...
      return '' +
        '<article class="summary-item">' +
          '<div class="summary-item__main">' +
            ...
            '<div class="summary-item__use-model">' + t("use_model_label") + ': ' + escapeHtml(formatUseModels(item)) + '</div>' +
